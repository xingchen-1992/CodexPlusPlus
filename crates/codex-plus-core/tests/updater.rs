...
fn update_zip_is_extracted_and_launches_root_setup() {
    ...
    let zip_path = dir
        .path()
        .join("CodexPlusOfficial-1.0.13-official.3-windows-x64.zip");
    let file = std::fs::File::create(&zip_path).unwrap();
    let mut zip = zip::ZipWriter::new(file);
    let options = zip::write::SimpleFileOptions::default();
    zip.start_file(
        "CodexPlusOfficial-1.0.13-official.3-windows-x64-setup.exe",
        options,
    )
    .unwrap();
    ...
    assert_eq!(
        ...
        Some("CodexPlusOfficial-1.0.13-official.3-windows-x64-setup.exe")
    );
