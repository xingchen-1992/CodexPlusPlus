        ...
        "downloads": [
            {
                ...
                "mirrorUrl": "https://www.leishen-ai.cn/node/node-v24.18.0-darwin-arm64.tar.gz",
                "sha256": "darwin-sha"
            },
            {
                "platform": "windows",
                ...
                "mirrorUrl": "https://www.leishen-ai.cn/node/node-v24.18.0-win-x64.zip",
                "sha256": "windows-sha"
            }
        ]
    });
...
fn build_codex_config_toml_contains_official_crs_defaults() {
    let config = build_codex_config_toml("https://www.leishen-ai.cn/openai");
...
fn build_codex_config_toml_escapes_control_characters() {
    let config = build_codex_config_toml("https://www.leishen-ai.cn/openai?note=line1\nline2");
    let parsed: toml::Value = toml::from_str(&config).unwrap();

    assert_eq!(
        parsed["model_providers"]["crs"]["base_url"].as_str(),
        Some("https://www.leishen-ai.cn/openai?note=line1\nline2")
    );
}
