...
test("Windows self-update prefers the dedicated updater over online and offline packages", () => {
  assert.match(updateSource, /fn is_windows_update_asset/);
  assert.match(updateSource, /fn is_windows_setup_asset/);
  assert.match(updateSource, /fn is_windows_full_package_asset/);
  assert.match(updateSource, /is_windows_update_asset\(name\)\s*\{\s*return 0;/);
  assert.match(updateSource, /is_windows_setup_asset\(name\)\s*\{\s*return 1;/);
  assert.match(updateSource, /is_windows_full_package_asset\(name\)\s*\{\s*return 2;/);
