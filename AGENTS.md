...
- 当前用户下载加速服务器为 `39.105.46.156`，域名 `https://leishenai.cn/`，Nginx 根目录 `/home/admin/design-site`，下载资产路径 `/home/admin/design-site/tools/codex-plus/releases/<version>/`。
- 上传到 `leishenai.cn` 的大 ZIP 和安装器必须先进 `.staging`，校验 sha256 和 size 后再发布到公开目录；发布后用 `curl -I` 和 1MB Range 请求验证 HTTPS、`Content-Length`、下载链路。
...
- `download-latest.json` 的用户下载 URL 可指向 `https://leishenai.cn/tools/codex-plus/releases/<version>/...`；`latest.json` 仍是管理工具自动更新专用，不要因为迁移用户下载大包而把 updater 入口混入官网下载页。
